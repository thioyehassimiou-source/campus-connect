...
class _ModernEnhancedScheduleScreenState extends ConsumerState<ModernEnhancedScheduleScreen> {
  ...
  Widget _buildCourseCard(Map<String, dynamic> course, bool isTeacher, bool isAdmin) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: course['color'].withValues(alpha: 0.3), width: 2),
      ),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
  ...
  Color _getStatusBgColor(String status) {
    switch (status) {
      case 'Annulé': return Colors.red.withValues(alpha: 0.1);
      case 'Rejeté': return Colors.red.withValues(alpha: 0.1);
      case 'Déplacé': return Colors.orange.withValues(alpha: 0.1);
      case 'En attente': return Colors.blue.withValues(alpha: 0.1);
  ...
  void _exportSchedule() async {
    ...
    final studentName = user?.fullName ?? 'Etudiant';
