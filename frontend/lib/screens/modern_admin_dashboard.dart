...
class _AdminDashboardHomeState extends ConsumerState<AdminDashboardHome> {
  ...
  Widget _buildUserRow(String name, String email, String role, String status, bool isActive, double totalWidth, String userId) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(bottom: BorderSide(color: Theme.of(context).dividerColor.withValues(alpha: 0.1), width: 1)),
      ),
      child: Row(
        children: [
...
class _AdminSettingsTabState extends ConsumerState<AdminSettingsTab> {
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                final response = await ApiService.changePassword(newPassword, token: token);
  ...
  Widget _buildSettingsItem(
    ...
    return ListTile(
      ...
      trailing: trailing ?? Icon(Icons.chevron_right, size: 20, color: Theme.of(context).iconTheme.color?.withValues(alpha: 0.5)),
      onTap: onTap,
    );
  }
}
