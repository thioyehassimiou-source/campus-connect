            ...
            Row(
              children: [
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                  decoration: BoxDecoration(
                    color: isOverdue ? const Color(0xFFEF4444).withValues(alpha: 0.1) : const Color(0xFFF1F5F9),
                    borderRadius: BorderRadius.circular(4),
                  ),
                  child: Text(
                              ...
                              onPressed: () {
                                if (sub.fileUrl != null) {
                                  DownloadService.downloadFile(
                                    sub.fileUrl!,
                                    sub.fileName ?? 'submission_${sub.id}',
                                    context,
                                  );
                                } else {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Aucun fichier associé')),
                                  );
                                }
                              },
                            ),
